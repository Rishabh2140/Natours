...
exports.resizeTourImages = catchAsync(async (req, res, next) => {
  // If there is no req.params.id (for create route), generate one so we can
  // name files consistently and (optionally) use the same _id when creating the doc.
  const uniqueId = req.params.id || new mongoose.Types.ObjectId().toHexString();

  // If id was missing, attach it so downstream code (and filenames) can use it.
  if (!req.params.id) req.params.id = uniqueId;

  // Also set req.body._id so createOne will create the document with this _id
  // (this makes it possible to create the tour and upload images in a single request).
  if (!req.body._id) req.body._id = uniqueId;

  // 1) Cover image
  if (req.files && req.files.imageCover) {
    // tour-<id>-<timestamp>-cover.jpeg
  ...
  // If neither imageCover nor images existed, we still just call next()
  next();
});
